...
const financial = {
    ...
    text: <div>
        ...
        <p>Any variable whose value changes over time in an uncertain way is said to follow a stochastic process. Stochastic processes can be either continous time or discrete time. A discrete time stochastic process is one where the variable value can change only at certain fixed points. A continous time stochastic process is when the value can change at any time. They can also be discrete variable or continous variable where the discrete variable can only take certain discrete values and continous can have any value in a specific range. For stock prices are restricted to discrete variable as they have a range of cents and discrete time as the stock market opens and closes. </p>

        <h3>Markov Property</h3>

          <p>The Markov process is a stochastic process where only the current value of a variable is relevant in predicting the future. Stock prices are usually assumed to follow a Markov process. Therefore if a stock is £50 now this is the only relevant piece of information for Markov processes for predicting the future value. The Markov property states that the probability distributions of hte price at any particular future time is not dependent on the particular path followed by the price in the past. <strong>Weak form efficiency</strong> states that past prices, histroical values and trends cannot predict future prices [Investopedia](https://www.investopedia.com/ask/answers/032615/what-are-differences-between-weak-strong-and-semistrong-versions-efficient-market-hypothesis.asp). This means that the current price of a stock contains all the information contained in a a record of past prices.</p>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       <p></p>
            ...
            <p>You can then formulate the dividend for absolute dividends as the Spot Price after ex-dividend date is the Spot Price before the ex-dividend date subtract the dividend payment size. Dividends can be continous or discrete they are usually discrete for a single stock where they are paid at fixed dates. For indexes however you can get multiple payments of dividends from the different stocks that make up the index this is more aligned to continous.</p>
