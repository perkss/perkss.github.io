...
const MathPreliminaries  = () => (
        ...
        <li>Give a proof that <Latex>$P(1)$</Latex> is true. Simply verify that the method works for the number <Latex>$1$</Latex>.</li>
        <li>Assume that the method works for an arbitrary number, <Latex>$n$ or sometimes $k$ is used but it means the same</Latex>.</li>
        <li>Give a proof that “if all of <Latex>$P(1), P (2), ..., P (n)$</Latex> are true, then <Latex>$P(n + 1)$</Latex> is also true”; this proof should be valid for any positive integer <Latex>$n$. Simply prove that if the method works for an arbitrary number $k$, then it work for the number $k+1$</Latex>, too.</li>
    ...
    <p>Mathematical induction is used as a conclusive proof for an infinite number of statements one for each <Latex>$n$</Latex>. The term induction is used as one must somehow define what is to be proved before one can apply the technique of mathematical induction.</p>
        ...
        <strong>Step 3: </strong> <Latex>And then prove that the equation is true for $n + 1$. So we substitue $n$ on both sides with $n + 1$ giving the below.</Latex>
         <p></p>
 ...
 <br />
        ...
        <Latex>   $= 1 + 3 + 5 + ... + (2n - 1) + (2n + 2 - 1)$</Latex> (simplify)<br /><br />
        ...
        <Latex>   $= (n + 1)^2$</Latex>(See math prerequisites we did this example of simplification from the line above.) <br /><br />

Brilliant we have simplified the left side to match the right side, by induction, for every positive integer <Latex>$n,
1 + 3 + 5 + ... + (2n - 1) = n^2$</Latex>. This is also proved visually using the square table in the book so please check that out. Some pointers it is the whole numer of squares up to the number so <Latex>$3$ is four squares as its the second step so $2 \times 2 - 1 = 3.$ Then you need to add the extra square from $1$ so that is $4$ which is $n^2$ so $2^2 = 4$ is equal to the added number of squares from the bottom left corner of $4$.</Latex> 
